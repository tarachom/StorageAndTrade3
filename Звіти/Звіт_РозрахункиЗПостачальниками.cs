...
";
            ...
            if (Фільтр.Контрагент.Length > 0)
            {
                query += isExistParent ? "AND" : "WHERE";
                query += $@"
РозрахункиЗПостачальниками.{РозрахункиЗПостачальниками_Const.Контрагент} = IN ('{string.Join("', '", Фільтр.Контрагент.Select(x => x.UniqueID.UGuid))}')
";
            }
            ...
            if (Фільтр.Валюта.Length > 0)
            {
                ...
                query += $@"
РозрахункиЗПостачальниками.{РозрахункиЗПостачальниками_Const.Валюта} IN ('{string.Join("', '", Фільтр.Валюта.Select(x => x.UniqueID.UGuid))}')
";
            }
