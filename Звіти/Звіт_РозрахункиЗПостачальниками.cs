...
";
            ...
            if (!Контрагент.Pointer.IsEmpty())
            {
                query += isExistParent ? "AND" : "WHERE";
                query += $@"
РозрахункиЗПостачальниками.{РозрахункиЗПостачальниками_Const.Контрагент} = '{Контрагент.Pointer.UnigueID}'
";
            }
            ...
            if (!Валюта.Pointer.IsEmpty())
            {
                ...
                query += $@"
РозрахункиЗПостачальниками.{РозрахункиЗПостачальниками_Const.Валюта} = '{Валюта.Pointer.UnigueID}'
";
            }
