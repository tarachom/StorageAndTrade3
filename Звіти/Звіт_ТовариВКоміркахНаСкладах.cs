    ...
    class Звіт_ТовариВКоміркахНаСкладах : VBox
    {
        ...
        HBox ВідобразитиФільтр(string typeReport, ПараметриФільтр Фільтр)
        {
            ...
            if (!Фільтр.СкладськеПриміщення.IsEmpty())
                text += "Складське приміщення: <b>" + Фільтр.СкладськеПриміщення.GetPresentation() + "</b>; ";

            if (!Фільтр.СкладськаКомірка.IsEmpty())
                text += "Складська комірка: <b>" + Фільтр.СкладськаКомірка.GetPresentation() + "</b>; ";
...
";
            ...
            ТипиДаних.Add("Пакування_Назва", new ПакуванняОдиниціВиміру_Pointer().GetBasis().Text);
            ТипиДаних.Add("Серія_Номер", new СеріїНоменклатури_Pointer().GetBasis().Text);
            ТипиДаних.Add("Приміщення_Назва", new СкладськіПриміщення_Pointer().GetBasis().Text);
            ТипиДаних.Add("Склад_Назва", new Склади_Pointer().GetBasis().Text);
            ТипиДаних.Add("Комірка_Назва", new СкладськіКомірки_Pointer().GetBasis().Text);
