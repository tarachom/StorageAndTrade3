...
";
            ...
            if (Фільтр.Контрагент.Length > 0)
            {
                query += isExistParent ? "AND" : "WHERE";
                query += $@"
РозрахункиЗКлієнтами.{РозрахункиЗКлієнтами_Const.Контрагент} IN ('{string.Join("', '", Фільтр.Контрагент.Select(x => x.UnigueID.UGuid))}')
";
            }
            ...
            if (Фільтр.Валюта.Length > 0)
            {
                ...
                query += $@"
РозрахункиЗКлієнтами.{РозрахункиЗКлієнтами_Const.Валюта} IN ('{string.Join("', '", Фільтр.Валюта.Select(x => x.UnigueID.UGuid))}')
";
            }
