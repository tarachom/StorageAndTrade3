...
{
    class Звіт_Закупівлі : ФормаЗвіт2
    {
        #region Filters
        ...
        const string КлючНалаштуванняКористувача = "Звіт.Закупівлі";
        ...
        async void Обороти()
        {
            ...
            string query = $@"
SELECT 
" + (Фільтр.ГрупуватиПоПеріоду ? $@"Закупівлі.{Закупівлі_Обороти_TablePart.Період} AS Період, TO_CHAR(Закупівлі.{Закупівлі_Обороти_TablePart.Період}, 'dd.mm.yyyy') AS Період_Назва," : "") + $@" 
    ...
    Закупівлі.{Закупівлі_Обороти_TablePart.Контрагент} AS Контрагент,
    Довідник_Контрагенти.{Контрагенти_Const.Назва} AS Контрагент_Назва,
    Закупівлі.{Закупівлі_Обороти_TablePart.Договір} AS Договір,
    ...
    Закупівлі.{Закупівлі_Обороти_TablePart.Номенклатура} AS Номенклатура, 
...
FROM 
    ...
    LEFT JOIN {Організації_Const.TABLE} AS Довідник_Організації ON Довідник_Організації.uid = 
        Закупівлі.{Закупівлі_Обороти_TablePart.Організація}
    LEFT JOIN {Склади_Const.TABLE} AS Довідник_Склади ON Довідник_Склади.uid = 
        Закупівлі.{Закупівлі_Обороти_TablePart.Склад}
    LEFT JOIN {Контрагенти_Const.TABLE} AS Довідник_Контрагенти ON Довідник_Контрагенти.uid = 
        Закупівлі.{Закупівлі_Обороти_TablePart.Контрагент}
    LEFT JOIN {ДоговориКонтрагентів_Const.TABLE} AS Довідник_ДоговориКонтрагентів ON Довідник_ДоговориКонтрагентів.uid = 
        Закупівлі.{Закупівлі_Обороти_TablePart.Договір}
    LEFT JOIN {Номенклатура_Const.TABLE} AS Довідник_Номенклатура ON Довідник_Номенклатура.uid = 
        Закупівлі.{Закупівлі_Обороти_TablePart.Номенклатура}
    LEFT JOIN {ХарактеристикиНоменклатури_Const.TABLE} AS Довідник_ХарактеристикиНоменклатури ON Довідник_ХарактеристикиНоменклатури.uid = 
        Закупівлі.{Закупівлі_Обороти_TablePart.ХарактеристикаНоменклатури}
...
";
            ...
            Звіт.ColumnSettings.Add("Кількість", new("Кількість", "", "", 1, ФункціїДляЗвітів.ФункціяДляКолонкиВідємнеЧислоЧервоним));
            Звіт.ColumnSettings.Add("Сума", new("Сума", "", "", 1, ФункціїДляЗвітів.ФункціяДляКолонкиВідємнеЧислоЧервоним));
            Звіт.ColumnSettings.Add("Собівартість", new("Собівартість", "", "", 1, ФункціїДляЗвітів.ФункціяДляКолонкиВідємнеЧислоЧервоним));
