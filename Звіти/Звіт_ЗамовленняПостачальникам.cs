...
";
            ...
            if (Фільтр.Номенклатура.Length > 0)
            {
                ...
                query += $@"
ЗамовленняПостачальникам.{ЗамовленняПостачальникам_Const.Номенклатура} IN ('{string.Join("', '", Фільтр.Номенклатура.Select(x => x.UniqueID.UGuid))}')
";
            }
            ...
            if (Фільтр.ХарактеристикиНоменклатури.Length > 0)
            {
                ...
                query += $@"
ЗамовленняПостачальникам.{ЗамовленняПостачальникам_Const.ХарактеристикаНоменклатури} IN ('{string.Join("', '", Фільтр.ХарактеристикиНоменклатури.Select(x => x.UniqueID.UGuid))}')
";
            }
            ...
            if (Фільтр.Склад.Length > 0)
            {
                ...
                query += $@"
ЗамовленняПостачальникам.{ЗамовленняПостачальникам_Const.Склад} IN ('{string.Join("', '", Фільтр.Склад.Select(x => x.UniqueID.UGuid))}')
";
            }
...
";
            ...
            //PDF
            {
                ...
                Звіт.PDFColumnSettings.Add("ЗамовленняПостачальнику_Назва", new("Замовлення постачальнику", 6));
                Звіт.PDFColumnSettings.Add("Номенклатура_Назва", new("Номенклатура", 5));
