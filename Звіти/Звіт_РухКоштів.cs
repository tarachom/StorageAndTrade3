...
";
            ...
            if (Фільтр.Каса.Length > 0)
            {
                ...
                query += $@"
Довідник_Каси.uid IN ('{string.Join("', '", Фільтр.Каса.Select(x => x.UniqueID.UGuid))}')
";
            ...
            if (Фільтр.Валюта.Length > 0)
            {
                ...
                query += $@"
Довідник_Валюти.uid IN ('{string.Join("', '", Фільтр.Валюта.Select(x => x.UniqueID.UGuid))}')
";
            }
...
";
            ...
            //Excel
            {
                ...
                Звіт.ExcelColumnSettings.Add("Каса_Назва", new("Каса"));
...
";
            ...
            if (!Фільтр.Організація.IsEmpty())
            {
                query += isExistParent ? "AND" : "WHERE";
                query += $@"
РухКоштів.{РухКоштів_Const.Організація} = '{Фільтр.Організація.UniqueID}'
";
            }
            ...
            if (Фільтр.Каса.Length > 0)
            {
                ...
                query += $@"
РухКоштів.{РухКоштів_Const.Каса} IN ('{string.Join("', '", Фільтр.Каса.Select(x => x.UniqueID.UGuid))}')
";
            ...
            if (Фільтр.Валюта.Length > 0)
            {
                ...
                query += $@"
РухКоштів.{РухКоштів_Const.Валюта} IN ('{string.Join("', '", Фільтр.Валюта.Select(x => x.UniqueID.UGuid))}')
";
            }
