...
";
            ...
            if (Фільтр.Контрагент.Length > 0)
            {
                ...
                query += $@"
КлієнтиТаПостачальники.Контрагент IN ('{string.Join("', '", Фільтр.Контрагент.Select(x => x.UniqueID.UGuid))}')
";
            }
            ...
            if (Фільтр.Валюта.Length > 0)
            {
                ...
                query += $@"
КлієнтиТаПостачальники.Валюта IN ('{string.Join("', '", Фільтр.Валюта.Select(x => x.UniqueID.UGuid))}')
";
            }
