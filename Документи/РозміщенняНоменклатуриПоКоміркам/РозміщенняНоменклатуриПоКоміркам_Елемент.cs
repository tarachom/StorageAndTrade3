...
namespace StorageAndTrade
{
    class РозміщенняНоменклатуриПоКоміркам_Елемент : VBox
    {
        ...
        void CreatePack1(HPaned hPaned)
        {
            ...
            hBoxNumberDataDoc.PackStart(new Label($"{РозміщенняНоменклатуриПоКоміркам_Const.FULLNAME} №:"), false, false, 5);
        ...
        bool Save()
        {
            ...
            catch (Exception ex)
            {
                ФункціїДляПовідомлень.ДодатиПовідомленняПроПомилку(DateTime.Now, "Запис",
                    РозміщенняНоменклатуриПоКоміркам_Objest.UnigueID.UGuid, "Документ", РозміщенняНоменклатуриПоКоміркам_Objest.Назва, ex.Message);
        ...
        void ReloadList()
        {
            if (PageList != null)
            {
                PageList.SelectPointerItem = РозміщенняНоменклатуриПоКоміркам_Objest.UnigueID;
        ...
        void OnSaveAndSpendClick(object? sender, EventArgs args)
        {
            ...
            //Закрити сторінку
            if (isSave && РозміщенняНоменклатуриПоКоміркам_Objest.Spend)
