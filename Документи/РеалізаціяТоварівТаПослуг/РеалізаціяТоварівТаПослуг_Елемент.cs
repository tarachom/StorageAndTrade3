...
namespace StorageAndTrade
{
    class РеалізаціяТоварівТаПослуг_Елемент : VBox
    {
        ...
        void CreatePack1(HPaned hPaned)
        {
            ...
            hBoxNumberDataDoc.PackStart(new Label($"{РеалізаціяТоварівТаПослуг_Const.FULLNAME} №:"), false, false, 5);
        ...
        bool Save()
        {
            ...
            try
            {
                isSave = РеалізаціяТоварівТаПослуг_Objest.Save();
            }
            catch (Exception ex)
            {
                ФункціїДляПовідомлень.ДодатиПовідомленняПроПомилку(DateTime.Now, "Запис",
                    РеалізаціяТоварівТаПослуг_Objest.UnigueID.UGuid, "Документ", РеалізаціяТоварівТаПослуг_Objest.Назва, ex.Message);
        ...
        void ReloadList()
        {
            if (PageList != null)
            {
                PageList.SelectPointerItem = РеалізаціяТоварівТаПослуг_Objest.UnigueID;
        ...
        void OnSaveAndSpendClick(object? sender, EventArgs args)
        {
            ...
            //Закрити сторінку
            if (isSave && РеалізаціяТоварівТаПослуг_Objest.Spend)
