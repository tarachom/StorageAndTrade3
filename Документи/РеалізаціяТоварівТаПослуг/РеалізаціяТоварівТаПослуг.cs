                ...
                if (Message.Request(Program.GeneralForm, "Копіювати?") == ResponseType.Yes)
                {
                    ...
                    foreach (TreePath itemPath in selectionRows)
                    {
                        ...
                        if (РеалізаціяТоварівТаПослуг_Objest.Read(new UnigueID(uid)))
                        {
                            ...
                            РеалізаціяТоварівТаПослуг_Objest_Новий.Автор = Program.Користувач;
                            РеалізаціяТоварівТаПослуг_Objest_Новий.Менеджер = Program.Користувач;
        ...
        void OnNewDocNaOsnovi_ПоверненняТоварівВідКлієнта(object? sender, EventArgs args)
        {
            if (TreeViewGrid.Selection.CountSelectedRows() != 0)
            {
                ...
                foreach (TreePath itemPath in selectionRows)
                {
                    ...
                    поверненняТоварівВідКлієнта_Objest.Автор = Program.Користувач;
                    поверненняТоварівВідКлієнта_Objest.Менеджер = Program.Користувач;
