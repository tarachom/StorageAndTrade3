...
{
    class ЗбіркаТоварівНаСкладі : VBox
    {
        public ЗбіркаТоварівНаСкладі_Pointer? SelectPointerItem { get; set; }
        public ЗбіркаТоварівНаСкладі_Pointer? DocumentPointerItem { get; set; }
        public System.Action<ЗбіркаТоварівНаСкладі_Pointer>? CallBack_OnSelectPointer { get; set; }

        TreeView TreeViewGrid;
        ComboBoxText ComboBoxPeriodWhere = new ComboBoxText();
        SearchControl2 ПошукПовнотекстовий = new SearchControl2();

        public ЗбіркаТоварівНаСкладі(bool IsSelectPointer = false) : base()
        {
            new VBox(false, 0);
            BorderWidth = 0;
            ...
            if (IsSelectPointer)
            {
                ...
                bEmptyPointer.Clicked += (object? sender, EventArgs args) =>
                {
                    if (CallBack_OnSelectPointer != null)
                        CallBack_OnSelectPointer.Invoke(new ЗбіркаТоварівНаСкладі_Pointer());
            ...
            TreeViewGrid = new TreeView(ТабличніСписки.ЗбіркаТоварівНаСкладі_Записи.Store);
        ...
        void OnRowActivated(object sender, RowActivatedArgs args)
        {
            if (TreeViewGrid.Selection.CountSelectedRows() != 0)
            {
                ...
                SelectPointerItem = new ЗбіркаТоварівНаСкладі_Pointer(unigueID);
            }
        }
        ...
        void OnButtonPressEvent(object? sender, ButtonPressEventArgs args)
        {
            if (args.Event.Type == Gdk.EventType.DoubleButtonPress && TreeViewGrid.Selection.CountSelectedRows() != 0)
            {
                ...
                if (TreeViewGrid.Model.GetIter(out iter, TreeViewGrid.Selection.GetSelectedRows()[0]))
                {
                    ...
                    {
                        if (CallBack_OnSelectPointer != null)
                            CallBack_OnSelectPointer.Invoke(new ЗбіркаТоварівНаСкладі_Pointer(new UnigueID(uid)));
            ...
            if (TreeViewGrid.Selection.CountSelectedRows() != 0)
            {
                if (Message.Request(Program.GeneralForm, "Видалити?") == ResponseType.Yes)
                {
                    ...
                    foreach (TreePath itemPath in selectionRows)
                    {
                        ...
                        ЗбіркаТоварівНаСкладі_Objest ЗбіркаТоварівНаСкладі_Objest = new ЗбіркаТоварівНаСкладі_Objest();
                        if (ЗбіркаТоварівНаСкладі_Objest.Read(new UnigueID(uid)))
                            ЗбіркаТоварівНаСкладі_Objest.Delete();
                        else
                            Message.Error(Program.GeneralForm, "Не вдалось прочитати!");
                    }
            ...
            if (TreeViewGrid.Selection.CountSelectedRows() != 0)
            {
                if (Message.Request(Program.GeneralForm, "Копіювати?") == ResponseType.Yes)
                {
                    ...
                    foreach (TreePath itemPath in selectionRows)
                    {
                        ...
                        if (ЗбіркаТоварівНаСкладі_Objest.Read(new UnigueID(uid)))
                        {
                            ЗбіркаТоварівНаСкладі_Objest ЗбіркаТоварівНаСкладі_Objest_Новий = ЗбіркаТоварівНаСкладі_Objest.Copy();
                            ЗбіркаТоварівНаСкладі_Objest_Новий.Назва += " *";
                            ЗбіркаТоварівНаСкладі_Objest_Новий.ДатаДок = DateTime.Now;
                            ЗбіркаТоварівНаСкладі_Objest_Новий.НомерДок = (++Константи.НумераціяДокументів.ЗбіркаТоварівНаСкладі_Const).ToString("D8");
                            ЗбіркаТоварівНаСкладі_Objest_Новий.Автор = Program.Користувач;
                            ЗбіркаТоварівНаСкладі_Objest_Новий.Save();

                            //Зчитати та скопіювати табличну частину Товари
                            ЗбіркаТоварівНаСкладі_Objest.Товари_TablePart.Read();
                            ЗбіркаТоварівНаСкладі_Objest_Новий.Товари_TablePart.Records = ЗбіркаТоварівНаСкладі_Objest.Товари_TablePart.Copy();
                            ЗбіркаТоварівНаСкладі_Objest_Новий.Товари_TablePart.Save(true);

                            SelectPointerItem = ЗбіркаТоварівНаСкладі_Objest_Новий.GetDocumentPointer();
                        }
                        else
                            Message.Error(Program.GeneralForm, "Не вдалось прочитати!");
                    }
        ...
        {
            ТабличніСписки.ЗбіркаТоварівНаСкладі_Записи.ДодатиВідбірПоПеріоду(Enum.Parse<Перелічення.ТипПеріодуДляЖурналівДокументів>(ComboBoxPeriodWhere.ActiveId));
            LoadRecords();
        }
            ...
            if (TreeViewGrid.Selection.CountSelectedRows() != 0)
            {
                ...
                if (TreeViewGrid.Model.GetIter(out iter, TreeViewGrid.Selection.GetSelectedRows()[0]))
                {
                    ...
                    Program.GeneralForm?.CreateNotebookPage($"Проводки", () =>
                    {
                        ...
                        page.CreateReport(new ЗбіркаТоварівНаСкладі_Pointer(new UnigueID(uid)));
        ...
        void OnExportXMLClick(object? sender, EventArgs arg)
        {
            if (TreeViewGrid.Selection.CountSelectedRows() != 0)
            {
                ...
                foreach (TreePath itemPath in selectionRows)
                {
                    ...
                    string pathToSave = System.IO.Path.Combine(AppContext.BaseDirectory, $"ЗбіркаТоварівНаСкладі{uid}.xml");
                    ЗбіркаТоварівНаСкладі_Export.ToXmlFile(new ЗбіркаТоварівНаСкладі_Pointer(new UnigueID(uid)), pathToSave);
                }
