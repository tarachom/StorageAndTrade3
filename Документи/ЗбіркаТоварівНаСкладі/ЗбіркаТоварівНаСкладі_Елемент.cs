...
namespace StorageAndTrade
{
    class ЗбіркаТоварівНаСкладі_Елемент : VBox
    {
        ...
        void CreatePack1(HPaned hPaned)
        {
            ...
            hBoxNumberDataDoc.PackStart(new Label($"{ЗбіркаТоварівНаСкладі_Const.FULLNAME} №:"), false, false, 5);
        ...
        bool Save()
        {
            ...
            try
            {
                isSave = ЗбіркаТоварівНаСкладі_Objest.Save();
            }
            catch (Exception ex)
            {
                ФункціїДляПовідомлень.ДодатиПовідомленняПроПомилку(DateTime.Now, "Запис",
                    ЗбіркаТоварівНаСкладі_Objest.UnigueID.UGuid, "Документ", ЗбіркаТоварівНаСкладі_Objest.Назва, ex.Message);
        ...
        void ReloadList()
        {
            if (PageList != null)
            {
                PageList.SelectPointerItem = ЗбіркаТоварівНаСкладі_Objest.UnigueID;
        ...
        void OnSaveAndSpendClick(object? sender, EventArgs args)
        {
            ...
            //Закрити сторінку
            if (isSave && ЗбіркаТоварівНаСкладі_Objest.Spend)
