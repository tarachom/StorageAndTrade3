...
{
    class ВнутрішнєСпоживанняТоварів_ТабличнаЧастина_Товари : ДокументТабличнаЧастина
    {
        ...
        void AddColumn()
        {
            ...
            //ХарактеристикаНоменклатури
            {
                TreeViewColumn column = new TreeViewColumn("Характеристика", new CellRendererText(), "text", (int)Columns.ХарактеристикаНоменклатури) { Resizable = true, MinWidth = 20, Visible = Системні.ВестиОблікПоХарактеристикахНоменклатури_Const };
            ...
            //КількістьФакт
            {
                ...
                TreeViewColumn column = new TreeViewColumn("Факт", cellNumber, "text", (int)Columns.КількістьФакт) { Resizable = true, Alignment = 1, MinWidth = 50 };
                column.SetCellDataFunc(cellNumber, new TreeCellDataFunc(NumericCellDataFunc));
        ...
        public override async ValueTask LoadRecords()
        {
            if (ЕлементВласник != null)
            {
                ...
                foreach (ВнутрішнєСпоживанняТоварів_Товари_TablePart.Record record in ЕлементВласник.Товари_TablePart.Records)
                {
        ...
        public override async ValueTask SaveRecords()
        {
            if (ЕлементВласник != null)
            {
                ...
                foreach (Запис запис in Записи)
                {
                    ЕлементВласник.Товари_TablePart.Records.Add(new ВнутрішнєСпоживанняТоварів_Товари_TablePart.Record()
                    {
                        UID = запис.ID,
