...
namespace StorageAndTrade
{
    class РахунокФактура_Елемент : VBox
    {
        ...
        void CreatePack1(HPaned hPaned)
        {
            ...
            hBoxNumberDataDoc.PackStart(new Label($"{РахунокФактура_Const.FULLNAME} №:"), false, false, 5);
        ...
        bool Save()
        {
            ...
            catch (Exception ex)
            {
                ФункціїДляПовідомлень.ДодатиПовідомленняПроПомилку(DateTime.Now, "Запис",
                    РахунокФактура_Objest.UnigueID.UGuid, "Документ", РахунокФактура_Objest.Назва, ex.Message);
        ...
        void ReloadList()
        {
            if (PageList != null)
            {
                PageList.SelectPointerItem = РахунокФактура_Objest.UnigueID;
        ...
        void OnSaveAndSpendClick(object? sender, EventArgs args)
        {
            ...
            //Закрити сторінку
            if (isSave && РахунокФактура_Objest.Spend)
