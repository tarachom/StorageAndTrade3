...
namespace StorageAndTrade
{
    public class ПрихіднийКасовийОрдер : ДокументЖурнал
    {
        ...
        {
            await ТабличніСписки.ПрихіднийКасовийОрдер_Записи.UpdateRecords(TreeViewGrid, recordsChanged);
        }

        protected override void FillFilterList(ListFilterControl filterControl)
        {
            ТабличніСписки.ПрихіднийКасовийОрдер_Записи.CreateFilter(TreeViewGrid, filterControl);
        }

        protected override async ValueTask OpenPageElement(bool IsNew, UniqueID? uniqueID = null)
        {
            await ПрихіднийКасовийОрдер_Функції.OpenPageElement(IsNew, uniqueID, CallBack_LoadRecords);
        }

        protected override async ValueTask SetDeletionLabel(UniqueID uniqueID)
        {
            await ПрихіднийКасовийОрдер_Функції.SetDeletionLabel(uniqueID);
        }

        protected override async ValueTask<UniqueID?> Copy(UniqueID uniqueID)
        {
            return await ПрихіднийКасовийОрдер_Функції.Copy(uniqueID);
        }

        protected override async ValueTask VersionsHistory(UniqueID uniqueID)
        {
            await СпільніФорми_ІсторіяЗміниДаних_Список.Сформувати(new ПрихіднийКасовийОрдер_Pointer(uniqueID).GetBasis());
        }
        ...
        protected override async ValueTask BeforeSetValue()
        {
            ...
            NotebookFunction.AddChangeFunc(Program.GeneralNotebook, Name, UpdateRecords, ПрихіднийКасовийОрдер_Const.POINTER);
        }
        ...
        {
            ПрихіднийКасовийОрдер_Pointer Вказівник = new ПрихіднийКасовийОрдер_Pointer(uniqueID);
        ...
        {
            await ПрихіднийКасовийОрдер_Друк.PDF(uniqueID);
        }
        ...
        async void НаОснові_РозхіднийКасовийОрдер(object? sender, EventArgs args)
        {
            ...
            {
                ПрихіднийКасовийОрдер_Objest? Обєкт = await new ПрихіднийКасовийОрдер_Pointer(uniqueID).GetDocumentObject(false);
