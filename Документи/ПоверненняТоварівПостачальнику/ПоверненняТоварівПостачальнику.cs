...
namespace StorageAndTrade
{
    public class ПоверненняТоварівПостачальнику : ДокументЖурнал
    {
        ...
        {
            await ПоверненняТоварівПостачальнику_Функції.OpenPageElement(IsNew, uniqueID, CallBack_LoadRecords);
        }

        protected override async ValueTask SetDeletionLabel(UniqueID uniqueID)
        {
            await ПоверненняТоварівПостачальнику_Функції.SetDeletionLabel(uniqueID);
        }

        protected override async ValueTask<UniqueID?> Copy(UniqueID uniqueID)
        {
            return await ПоверненняТоварівПостачальнику_Функції.Copy(uniqueID);
        }

        protected override async ValueTask VersionsHistory(UniqueID uniqueID)
        {
            await СпільніФорми_ІсторіяЗміниДаних_Список.Сформувати(new ПоверненняТоварівПостачальнику_Pointer(uniqueID).GetBasis());
        }
        ...
        {
            ПоверненняТоварівПостачальнику_Pointer Вказівник = new ПоверненняТоварівПостачальнику_Pointer(uniqueID);
        ...
        {
            await ПоверненняТоварівПостачальнику_Друк.PDF(uniqueID);
        }
