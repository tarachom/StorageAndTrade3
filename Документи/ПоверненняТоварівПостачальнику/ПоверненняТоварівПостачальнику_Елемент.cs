...
namespace StorageAndTrade
{
    class ПоверненняТоварівПостачальнику_Елемент : VBox
    {
        ...
        void CreatePack1(HPaned hPaned)
        {
            ...
            hBoxNumberDataDoc.PackStart(new Label($"{ПоверненняТоварівПостачальнику_Const.FULLNAME} №:"), false, false, 5);
        ...
        bool Save()
        {
            ...
            try
            {
                isSave = ПоверненняТоварівПостачальнику_Objest.Save();
            }
            catch (Exception ex)
            {
                ФункціїДляПовідомлень.ДодатиПовідомленняПроПомилку(DateTime.Now, "Запис",
                    ПоверненняТоварівПостачальнику_Objest.UnigueID.UGuid, "Документ", ПоверненняТоварівПостачальнику_Objest.Назва, ex.Message);
        ...
        void ReloadList()
        {
            if (PageList != null)
            {
                PageList.SelectPointerItem = ПоверненняТоварівПостачальнику_Objest.UnigueID;
        ...
        void OnSaveAndSpendClick(object? sender, EventArgs args)
        {
            ...
            //Закрити сторінку
            if (isSave && ПоверненняТоварівПостачальнику_Objest.Spend)
