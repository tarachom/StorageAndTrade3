                ...
                if (Message.Request(Program.GeneralForm, "Копіювати?") == ResponseType.Yes)
                {
                    ...
                    foreach (TreePath itemPath in selectionRows)
                    {
                        ...
                        if (ПереміщенняТоварівНаСкладі_Objest.Read(new UnigueID(uid)))
                        {
                            ...
                            ПереміщенняТоварівНаСкладі_Objest_Новий.Коментар = "Копія: " + ПереміщенняТоварівНаСкладі_Objest.Назва;
        ...
        void OnExportXMLClick(object? sender, EventArgs arg)
        {
            if (TreeViewGrid.Selection.CountSelectedRows() != 0)
            {
                ...
                foreach (TreePath itemPath in selectionRows)
                {
                    ...
                    string pathToSave = System.IO.Path.Combine(AppContext.BaseDirectory, $"ПереміщенняТоварівНаСкладі{uid}.xml");
                    ПереміщенняТоварівНаСкладі_Export.ToXmlFile(new ПереміщенняТоварівНаСкладі_Pointer(new UnigueID(uid)), pathToSave);
                }
