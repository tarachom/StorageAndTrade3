...
namespace StorageAndTrade
{
    class ПереміщенняТоварівНаСкладі_Елемент : ДокументЕлемент
    {
        ...
        {
            ПереміщенняТоварівНаСкладі_Objest.UnigueIDChanged += UnigueIDChanged;
            ПереміщенняТоварівНаСкладі_Objest.CaptionChanged += CaptionChanged;
        ...
        protected override async ValueTask<bool> Save()
        {
            ...
            {
                ФункціїДляПовідомлень.ДодатиПовідомлення(ПереміщенняТоварівНаСкладі_Objest.GetBasis(), Caption, ex);
            }
        ...
        protected override async ValueTask<bool> SpendTheDocument(bool spendDoc)
        {
            if (spendDoc)
            {
                ...
                if (!isSpend)
                    ФункціїДляПовідомлень.ПоказатиПовідомлення(ПереміщенняТоварівНаСкладі_Objest.UnigueID);
