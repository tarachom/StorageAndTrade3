...
namespace StorageAndTrade
{
    class ПоверненняТоварівВідКлієнта_Елемент : VBox
    {
        ...
        void CreatePack1(HPaned hPaned)
        {
            ...
            hBoxNumberDataDoc.PackStart(new Label($"{ПоверненняТоварівВідКлієнта_Const.FULLNAME} №:"), false, false, 5);
        ...
        bool Save()
        {
            ...
            try
            {
                isSave = ПоверненняТоварівВідКлієнта_Objest.Save();
            }
            catch (Exception ex)
            {
                ФункціїДляПовідомлень.ДодатиПовідомленняПроПомилку(DateTime.Now, "Запис",
                    ПоверненняТоварівВідКлієнта_Objest.UnigueID.UGuid, "Документ", ПоверненняТоварівВідКлієнта_Objest.Назва, ex.Message);
        ...
        void ReloadList()
        {
            if (PageList != null)
            {
                PageList.SelectPointerItem = ПоверненняТоварівВідКлієнта_Objest.UnigueID;
        ...
        void OnSaveAndSpendClick(object? sender, EventArgs args)
        {
            ...
            //Закрити сторінку
            if (isSave && ПоверненняТоварівВідКлієнта_Objest.Spend)
