                ...
                if (Message.Request(Program.GeneralForm, "Копіювати?") == ResponseType.Yes)
                {
                    ...
                    foreach (TreePath itemPath in selectionRows)
                    {
                        ...
                        if (ПоверненняТоварівВідКлієнта_Objest.Read(new UnigueID(uid)))
                        {
                            ...
                            ПоверненняТоварівВідКлієнта_Objest_Новий.Автор = Program.Користувач;
                            ПоверненняТоварівВідКлієнта_Objest_Новий.Менеджер = Program.Користувач;
        ...
        void OnExportXMLClick(object? sender, EventArgs arg)
        {
            if (TreeViewGrid.Selection.CountSelectedRows() != 0)
            {
                ...
                foreach (TreePath itemPath in selectionRows)
                {
                    ...
                    string pathToSave = System.IO.Path.Combine(AppContext.BaseDirectory, $"ПоверненняТоварівВідКлієнта{uid}.xml");
                    ПоверненняТоварівВідКлієнта_Export.ToXmlFile(new ПоверненняТоварівВідКлієнта_Pointer(new UnigueID(uid)), pathToSave);
                }
