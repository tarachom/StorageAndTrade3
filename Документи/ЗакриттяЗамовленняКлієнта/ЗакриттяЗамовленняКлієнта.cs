...
namespace StorageAndTrade
{
    public class ЗакриттяЗамовленняКлієнта : ДокументЖурнал
    {
        public ЗакриттяЗамовленняКлієнта() : base()
        {
            ...
            ТабличніСписки.ЗакриттяЗамовленняКлієнта_Записи.Сторінки(TreeViewGrid, new Сторінки.Налаштування() { PageSize = 300, Тип = Сторінки.ТипЖурналу.Документи });
        }
        ...
        {
            return ТабличніСписки.ЗакриттяЗамовленняКлієнта_Записи.CreateFilter(TreeViewGrid, () => PagesShow(LoadRecords_OnFilter));
        }
