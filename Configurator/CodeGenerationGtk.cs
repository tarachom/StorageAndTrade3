 ...
 * Дата конфігурації: 21.01.2023 15:08:07
 *
 */
...
namespace StorageAndTrade_1_0.Довідники.ТабличніСписки
{
    ...
    public class СкладськіКомірки_Записи
    {
        ...
        public static void AddColumns(TreeView treeView)
        {
            ...
            treeView.AppendColumn(new TreeViewColumn("Стелаж", new CellRendererText() { Xpad = 4 }, "text", 6) { SortColumnId = 6 } ); /*Стелаж*/
            ...
            while (ПрихіднийКасовийОрдер_Select.MoveNext())
            {
                ...
                if (cur != null)
                {
                    ПрихіднийКасовийОрдер_Записи Record = new ПрихіднийКасовийОрдер_Записи
                    {
                        ...
                        ГосподарськаОперація = ((Перелічення.ГосподарськіОперації)(cur.Fields?[ПрихіднийКасовийОрдер_Const.ГосподарськаОперація]! != DBNull.Value ? cur.Fields?[ПрихіднийКасовийОрдер_Const.ГосподарськаОперація]! : 0)).ToString(), /**/
            ...
            while (РозхіднийКасовийОрдер_Select.MoveNext())
            {
                ...
                if (cur != null)
                {
                    РозхіднийКасовийОрдер_Записи Record = new РозхіднийКасовийОрдер_Записи
                    {
                        ...
                        ГосподарськаОперація = ((Перелічення.ГосподарськіОперації)(cur.Fields?[РозхіднийКасовийОрдер_Const.ГосподарськаОперація]! != DBNull.Value ? cur.Fields?[РозхіднийКасовийОрдер_Const.ГосподарськаОперація]! : 0)).ToString(), /**/
