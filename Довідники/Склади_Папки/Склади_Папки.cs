...
namespace StorageAndTrade
{
    public class Склади_Папки : ДовідникЖурнал
    {
        ...
        {
            await Склади_Папки_Функції.OpenPageElement(IsNew, uniqueID, CallBack_LoadRecords, null);
        }

        protected override async ValueTask SetDeletionLabel(UniqueID uniqueID)
        {
            await Склади_Папки_Функції.SetDeletionLabel(uniqueID);
        }

        protected override async ValueTask<UniqueID?> Copy(UniqueID uniqueID)
        {
            return await Склади_Папки_Функції.Copy(uniqueID);
        }

        protected override async ValueTask VersionsHistory(UniqueID uniqueID)
        {
            await СпільніФорми_ІсторіяЗміниДаних_Список.Сформувати(new Склади_Папки_Pointer(uniqueID).GetBasis());
        }
