...
namespace StorageAndTrade
{
    public class ПартіяТоварівКомпозит : ДовідникЖурнал
    {
        ...
        {
            await ПартіяТоварівКомпозит_Функції.OpenPageElement(IsNew, uniqueID, CallBack_LoadRecords, null);
        }
        ...
        {
            return await ПартіяТоварівКомпозит_Функції.Copy(uniqueID);
        }

        protected override async ValueTask VersionsHistory(UniqueID uniqueID)
        {
            await СпільніФорми_ІсторіяЗміниДаних_Список.Сформувати(new ПартіяТоварівКомпозит_Pointer(uniqueID).GetBasis());
        }
