...
namespace StorageAndTrade
{
    public class ПакуванняОдиниціВиміру : ДовідникЖурнал
    {
        ...
        {
            await ТабличніСписки.ПакуванняОдиниціВиміру_Записи.UpdateRecords(TreeViewGrid, recordsChanged);
        }
        ...
        {
            await ПакуванняОдиниціВиміру_Функції.OpenPageElement(IsNew, uniqueID, CallBack_LoadRecords, null);
        }

        protected override async ValueTask SetDeletionLabel(UniqueID uniqueID)
        {
            await ПакуванняОдиниціВиміру_Функції.SetDeletionLabel(uniqueID);
        }

        protected override async ValueTask<UniqueID?> Copy(UniqueID uniqueID)
        {
            return await ПакуванняОдиниціВиміру_Функції.Copy(uniqueID);
        }

        protected override async ValueTask VersionsHistory(UniqueID uniqueID)
        {
            await СпільніФорми_ІсторіяЗміниДаних_Список.Сформувати(new ПакуванняОдиниціВиміру_Pointer(uniqueID).GetBasis());
        }

        protected override async ValueTask BeforeSetValue()
        {
            NotebookFunction.AddChangeFunc(Program.GeneralNotebook, Name, UpdateRecords, ПакуванняОдиниціВиміру_Const.POINTER);
