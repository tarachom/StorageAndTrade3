    ...
    public class ТипорозміриКомірок : ДовідникЖурнал
    {
        ...
        protected override async ValueTask<(string Name, Func<Widget>? FuncWidget, System.Action? SetValue)> OpenPageElement(bool IsNew, UnigueID? unigueID = null)
        {
            ...
            if (!IsNew && unigueID != null)
                if (!await page.ТипорозміриКомірок_Objest.Read(unigueID))
                {
                    Message.Error(Program.GeneralForm, "Не вдалось прочитати!");
                    return ("", null, null);
                }

            return (IsNew ? ТипорозміриКомірок_Const.FULLNAME : page.ТипорозміриКомірок_Objest.Назва, () => page, page.SetValue);
        }
