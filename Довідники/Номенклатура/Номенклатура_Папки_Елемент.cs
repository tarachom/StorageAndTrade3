...
namespace StorageAndTrade
{
    class Номенклатура_Папки_Елемент : VBox
    {
        ...
        void Save(bool closePage = false)
        {
            ...
            try
            {
                isSave = Номенклатура_Папки_Objest.Save();
            }
            catch (Exception ex)
            {
                ФункціїДляПовідомлень.ДодатиПовідомленняПроПомилку(DateTime.Now, "Запис",
                    Номенклатура_Папки_Objest.UnigueID.UGuid, "Довідники", Номенклатура_Папки_Objest.Назва, ex.Message);
            ...
            if (closePage)
                ...
                Program.GeneralForm?.RenameCurrentPageNotebook($"{Номенклатура_Папки_Objest.Назва}");

            if (CallBack_OnSelectPointer != null)
                CallBack_OnSelectPointer.Invoke(Номенклатура_Папки_Objest.GetDirectoryPointer());
