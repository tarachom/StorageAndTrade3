...
namespace StorageAndTrade
{
    class Контрагенти_Папки_Елемент : VBox
    {
        ...
        void Save(bool closePage = false)
        {
            ...
            try
            {
                isSave = Контрагенти_Папки_Objest.Save();
            }
            catch (Exception ex)
            {
                ФункціїДляПовідомлень.ДодатиПовідомленняПроПомилку(DateTime.Now, "Запис",
                    Контрагенти_Папки_Objest.UnigueID.UGuid, "Довідники", Контрагенти_Папки_Objest.Назва, ex.Message);
            ...
            if (closePage)
                ...
                Program.GeneralForm?.RenameCurrentPageNotebook($"{Контрагенти_Папки_Objest.Назва}");
